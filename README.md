...
git clone https://github.com/segupa03/groundwater-daily-estimation-tsd
...
# Load data
loader = DataLoader("data/groundwater_data.xlsx")
...
CREATE TABLE WaterLevels (
    Year INTEGER,           -- Treatment unit number or Year 
    Date INTEGER,           -- Julian day
    Basin INTEGER,          -- Treatment unit number (redundant with A)
    Well TEXT,              -- Well identifier
    WaterLevel REAL         -- Water level measurement
);
```
...
CREATE TABLE ManualMeasurements (
    Year INTEGER,          -- Treatment unit number
    Point TEXT,            -- Well identifier
    Day INTEGER            -- Julian day
    WaterLevel REAL        -- Water level, manual measurement
);
```
...
CREATE TABLE WellCoordinates (
    Basin INTEGER,         -- Treatment unit number
    Well TEXT,             -- Well identifier
    X REAL,                -- X coordinate
    Y REAL                 -- Y coordinate
);
...
```csv
Well_ID,Date,TreatmentUnit,WaterLevel,X,Y
Well_001,2022-01-01,1,45.2,123.45,67.89
Well_001,2022-01-02,1,45.1,123.45,67.89
...
```
